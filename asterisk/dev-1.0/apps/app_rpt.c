...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 150383 $")
...
char s1[256];
 static char *cs_keywords[] = {"rptena","rptdis","apena","apdis","lnkena","lnkdis","totena","totdis","skena","skdis",
	...
	if (init)
	{
		...
		rpt_vars[n].rxtele.qe_forw = &rpt_vars[n].rxtele;
		rpt_vars[n].rxtele.qe_back = &rpt_vars[n].rxtele;
		rpt_vars[n].links.next = &rpt_vars[n].links;
...
struct	ast_channel *mychannel,*rchannel;
int id_malloc, vmajor, vminor, m;
char *p,*ct,*ct_copy,*ident, *nodename,*cp,confc;
time_t t;
#ifdef	NEW_ASTERISK
...
struct rpt_rxtelem *rp;
	...
	confc = ( ((mytele->mode == ID) ||
		   (mytele->mode == IDTALKOVER) ||
		   (mytele->mode == UNKEY) ||
		   (mytele->mode == TAILMSG) ||
		   ...
		   (mytele->mode == LOCALPLAY) ) ? 'T' : 'C');

	mytele->uid = ++myrpt->uid;
	mytele->cmode = confc;
	sprintf(mystr,"%c-%s-%ld",confc,myrpt->name,mytele->uid);
	/* allocate a pseudo-channel thru asterisk */
	mychannel = ast_request("locloop",AST_FORMAT_SLINEAR,mystr,NULL);
	if (!mychannel)
	{
...
#endif
	ast_setstate(mychannel,AST_STATE_UP);
	/* allocate this last channels partner */
	rchannel = ast_request("locloop",AST_FORMAT_SLINEAR,mystr,NULL);
	if (!rchannel)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
		rpt_mutex_lock(&myrpt->lock);
	...
	if (mychannel->cdr)
		ast_set_flag(mychannel->cdr,AST_CDR_FLAG_POST_DISABLED);
#endif
	ast_setstate(rchannel,AST_STATE_UP);
	rpt_mutex_lock(&myrpt->lock);
	mytele->chan = mychannel;
	mytele->rchan = rchannel;
	rp = malloc(sizeof(struct rpt_rxtelem));
	if (!rp)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
		rpt_mutex_lock(&myrpt->lock);
	...
	switch(mytele->mode)
	{
		...
		*/
		...
		{
			if (mytele->rxtelem)  mytele->rxtelem->cmode = 'C';
		...
		if (myrpt->lastunit)
		{
			...
			ast_safe_sleep(mychannel,200);
			if (mytelem->rxtelem)  mytelem->rxtelem.cmode = 'T';
		...
		*/
		...
		while(l != &myrpt->links)
		{
			...
			if (!l1)
			{
				...
				ast_softhangup(rchannel,AST_SOFTHANGUP_DEV);
				pthread_exit(NULL);
			}
...
struct ast_channel *mychannel,*hischannel;
struct tone_zone_sound *ts;
char	mystr[100];

	myrpt->mydtmf = 0;
	sprintf(mystr,"Phone-%s",myrpt->name);
	/* allocate a pseudo-channel thru asterisk */
	mychannel = ast_request("locloop",AST_FORMAT_SLINEAR,mystr,NULL);
	if (!mychannel)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain locloop channel\n");
		pthread_exit(NULL);
	}
#ifdef	AST_CDR_FLAG_POST_DISABLED
	if (mychannel->cdr)
...
#endif
	ast_setstate(mychannel,AST_STATE_UP);
	hischannel = ast_request("locloop",AST_FORMAT_SLINEAR,mystr,NULL);
	if (!hischannel)
	{
		fprintf(stderr,"rpt:Sorry unable to obtain locloop channel\n");
		pthread_exit(NULL);
...
#ifdef	AST_CDR_FLAG_POST_DISABLED
	if (hischannel->cdr)
		ast_set_flag(hischannel->cdr,AST_CDR_FLAG_POST_DISABLED);
#endif
	ast_setstate(hischannel,AST_STATE_UP);
	rpt_mutex_lock(&myrpt->lock);
	myrpt->pchannel = hischannel;
	rpt_mutex_unlock(&myrpt->lock);
	if (!myrpt->patchquiet)
	{
		ts = ast_get_indication_tone(mychannel->zone,"dial");
		ast_playtones_start(mychannel,0, ts->data, 0);
	}
	stopped = 0;
	congstarted = 0;
	dialtimer = 0;
	...
	while ((myrpt->callmode == 1) || (myrpt->callmode == 4))
	{
		...
		if (myrpt->callmode == 4)
		{
			if(!congstarted){
				...
				ts = ast_get_indication_tone(mychannel->zone,"congestion");
				ast_playtones_start(mychannel,0, ts->data, 0);
			}
		}
...
#ifdef	NEW_ASTERISK
			ast_senddigit(mychannel,myrpt->mydtmf,0);
#else
			ast_senddigit(mychannel,myrpt->mydtmf);
#endif
			myrpt->mydtmf = 0;
		}
		usleep(MSWAIT * 1000);
	...
	channel_revert(myrpt);
	rpt_mutex_unlock(&myrpt->lock);
	if (hischannel) ast_hangup(hischannel);
	pthread_exit(NULL);
}
...
static int connect_link(struct rpt *myrpt, char* node, int mode, int perma)
{
	...
	if ((strncasecmp(s1,"iax2/", 5) == 0) ||
#ifdef ALLOW_LOCAL_CHANNELS
	    (strncasecmp(s1,"local/", 6) == 0) ||
#endif
...
int tailmessagequeued,ctqueued,dtmfed,lastmyrx,localmsgqueued,nullfd;
...
char tmpstr[300],lstr[MAXLINKLIST];
	...
	*tele++ = 0;
	if (!strncasecmp(tmpstr,"zap",3)) myrpt->iszap = 1;
	...
	voxinit_rpt(myrpt,1);
	myrpt->wasvox = 0;
	rpt_mixer_reset(&myrpt->conf);
	rpt_mixer_reset(&myrpt->txconf);
...
/* printf("totx is %d for node %s\n",totx,myrpt->name); */
		...
		if (totx && (!lasttx))
		{
			...
			if (myrpt->p.archivedir)
			{
				...
				strftime(mydate,sizeof(mydate) - 1,"%Y%m%d%H%M%S",
					localtime(&myt));
				sprintf(myfname,"mixmonitor start %s %s/%s/%s.wav49 a",myrpt->txchannel->name,
				...
				if (myrpt->p.monminblocks)
				{
					...
					if (blocksleft >= myrpt->p.monminblocks)
						ast_cli_command(nullfd,myfname);
		...
		n = 0;
		if (myrpt->timingchannel) cs[n++] = myrpt->timingchannel;
		cs[n++] = myrpt->rxchannel;
		if (myrpt->txchannel != myrpt->rxchannel) cs[n++] = myrpt->txchannel;
		if (myrpt->callmode && myrpt->pchannel) cs[n++] = myrpt->pchannel;
		l = myrpt->links.next;
		while(l != &myrpt->links)
		{
			if ((!l->killme) && (!l->disctime) && l->chan && l->ready)
			{
				cs[n++] = l->chan;
			}
			l = l->next;
		}
		rxtelem = myrpt->rxtele.qe_forw;
		while(rxtelem != &myrpt->rxtele)
		{
			if (rxtelem->chan) cs[n++] = rxtelem->chan;
			rxtelem = rxtelem->qe_forw;
		}
		...
		ms = 0;
		if (myrpt->timingchannel) ms = 1;
		for(x = 0; x < (n - ms); x++)
		{
			int s = -(-x - myrpt->scram - 1) % (n - ms);
			cs1[x + ms] = cs[s + ms];
		}
		if (ms) cs1[0] = cs[0];
		ms = MSWAIT;
		...
		if (who == myrpt->timingchannel) /* if it was a read from tx */
		{
			f = ast_read(myrpt->timingchannel);
			if (!f)
			{
				if (debug) printf("@@@@ rpt:Hung Up\n");
				break;
		...
		if (myrpt->pchannel && (who == myrpt->pchannel)) /* if it was a read from phone */
		{
			...
			if (f->frametype == AST_FRAME_VOICE)
			{
				rpt_mutex_lock(&myrpt->lock);
				if (!myrpt->pcqflag)
					memcpy(myrpt->pcq,f->data,f->datalen);
				myrpt->pcqflag = 1;
				rpt_mutex_unlock(&myrpt->lock);
		...
		while(rxtelem != &myrpt->rxtele)
		{
			if (rxtelem->chan && (who == rxtelem->chan))
			{
				f = ast_read(rxtelem->chan);
				if (!f)
				{
					if (debug) printf("@@@@ rpt:Hung Up\n");
	...
	myrpt->lastf2 = NULL;
	ast_hangup(myrpt->rxchannel);
	if (myrpt->pchannel) ast_hangup(myrpt->pchannel);
	sprintf(tmpstr,"mixmonitor stop %s",myrpt->txchannel->name);
	ast_cli_command(nullfd,tmpstr);
	rpt_mutex_lock(&myrpt->lock);
	l = myrpt->links.next;
...
char *this,*val,gotfirst = 0;
	...
	{
		if (rpt_vars[n].remote) continue;
		if ((!gotfirst) && (!strncasecmp(rpt_vars[n].rxchanname,"zap",3)))
		{
			ast_log(LOG_NOTICE,"Setup node %s chan %s as first one\n",
				rpt_vars[n].name,rpt_vars[n].rxchanname);
			rpt_vars[n].firstone = 1;
			gotfirst = n + 1;
		}
	}
	if (!gotfirst)
	{
		rpt_vars[0].timingchannel = ast_request("zap",AST_FORMAT_SLINEAR,"pseudo",NULL);
		if (!rpt_vars[0].timingchannel)
		{
			fprintf(stderr,"rpt:Sorry unable to obtain pseudo channel\n");
			pthread_exit(NULL);
		}
		ast_set_read_format(rpt_vars[0].timingchannel,AST_FORMAT_SLINEAR);
		ast_set_write_format(rpt_vars[0].timingchannel,AST_FORMAT_SLINEAR);
#ifdef	AST_CDR_FLAG_POST_DISABLED
		if (rpt_vars[0].timingchannel->cdr)
			ast_set_flag(rpt_vars[0].timingchannel->cdr,AST_CDR_FLAG_POST_DISABLED);
#endif
		ast_log(LOG_NOTICE,"Setup Zap pseudo channel for timing\n");
	}
	usleep(500000);
	time(&starttime);
	for(;;)
...
static int rpt_exec(struct ast_channel *chan, void *data)
{
	...
	int ismuted,dtmfed,phone_vox = 0, phone_monitor = 0, x;
#ifdef	OLD_ASTERISK
	struct localuser *u;
#endif
	...
	struct ast_frame *f,*f1,*f2;
	struct ast_channel *who;
	struct ast_channel *cs[100],*cs1[100];
	struct rpt_rxtelem *rxtelem;
	struct	rpt_link *l;
	ZT_PARAMS par;
	int ms,elap,nullfd;
	...
	if (debug)
	{
		...
		if (memp > 0) ast_log(LOG_DEBUG,"memp=%s \n",memp);
	...
	*tele++ = 0;
	if (!strncasecmp(myrpt->rxchanname,"zap",3)) myrpt->iszap = 1;
	...
	*--tele = '/';
	...
	myrpt->rxtele.qe_forw = &myrpt->rxtele;
	myrpt->rxtele.qe_back = &myrpt->rxtele;
	myrpt->newkey = 0;
	rpt_mutex_unlock(&myrpt->lock);
	ast_set_write_format(chan, AST_FORMAT_SLINEAR);
	ast_set_read_format(chan, AST_FORMAT_SLINEAR);
	...
	if (myrpt->iszap && myrpt->remote && (myrpt->rxchannel == myrpt->txchannel))
	{
		i = 128;
...
#endif
{
	...
	ast_cli_unregister(&cli_localplay); /* JIM */
